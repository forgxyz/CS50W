...
def api(isbn):
    """Allow external calls to download bookguru review data."""
    book = db.execute(f"SELECT * FROM books WHERE isbn = :isbn", {"isbn": isbn}).first()
    if len(book) == 0:
        return "error, ISBN not found"
    reviews = db.execute(f"SELECT COUNT(stars) AS count, AVG(stars)::real AS avg FROM {REV_TABLE} WHERE isbn = :isbn", {"isbn": isbn}).first()
    db.commit()
    return jsonify(title=book.title, author=book.author, year=book.year, isbn=isbn, review_count=reviews.count, average_rating=reviews.avg)
...
def login():
    # load login screen if get request
    if request.method == 'GET':
        if session.get("user_id") != None:
            return redirect("/")
        return render_template("login.html")

    # otherwise, post/form submission
